...
public struct Serializer {
  ...
  public static func unserialize(data: NSData) throws -> Any? {
    do {
  ...
  public static func unserialize(a: AnyObject) -> Any? {
    if isBaseType(a) {
      return a
    }
    if isClass(a) {
    ...
    if isList(a) {
      if let array = a as? NSArray {
        var ret: [Any] = []
        for element in array {
          if let element = unserialize(element) {
            ret.append(element)
          }
        }
        return ret
        ...
        var ret: [String: Any] = [:]
        for (key, value) in map {
          if let value = unserialize(value) {
            ret[key] = value
          }
        }
        return ret
